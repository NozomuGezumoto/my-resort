// ============================================
// Hotel Map Component
// 一泊8万円以上の高級ホテルを世界地図に表示
// データはローカル JSON（getLuxuryHotelPins）
// ============================================
...
export type HotelStatusFilter = 'all' | 'visited' | 'wantToGo';
...
export default function HotelMap() {
  ...
  const handlePinPress = useCallback((hotel: HotelPin) => {
  ...
  useEffect(() => {
    if (
      ...
      const coordinates = filteredHotels.map((hotel) => ({ latitude: hotel.lat, longitude: hotel.lng }));
    ...
    <View style={styles.container}>
      ...
      >
        {filteredHotels.map((hotel) => {
          const visited = visitedHotels.includes(hotel.id);
          const wantToGo = wantToGoHotels.includes(hotel.id);
          const pinKind = visited ? 'visited' : wantToGo ? 'wantToGo' : 'default';
          return (
            <Marker
              key={`${hotel.id}-${pinKind}`}
              coordinate={{ latitude: hotel.lat, longitude: hotel.lng }}
              onPress={() => handlePinPress(hotel)}
              tracksViewChanges={visited || wantToGo}
            >
              <View
                style={[
                  styles.pinContainer,
                  pinKind === 'visited' && styles.hotelPinVisited,
                  pinKind === 'wantToGo' && styles.hotelPinWantToGo,
                  pinKind === 'default' && styles.hotelPin,
                ]}
              >
                <Ionicons
                  name={
